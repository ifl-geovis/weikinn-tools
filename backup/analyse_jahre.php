...
foreach ($files as $importfilename) {
	...
	// liefert nicht die ANzahl der besetzten Zellen zurück sondern meist viel mehr
...
function extractNames(&$dataArray,$ort,$year) {
	$ort = explode("//",$ort);
	
	foreach( $ort as $name) {
		$name = trim($name);
		if ($year) {
			if (!isset($dataArray[$name])) $dataArray[$name] = array();
			@$dataArray[$name][$year]++;
...
$f = fopen($resultfolder."namen_1.txt",'w');
fwrite( $f, implode("\n",array_keys($names)) );
fclose($f);
$names = array();
...
$f = fopen($resultfolder."namen_2.txt",'w');
fwrite( $f, implode("\n",array_keys($names)) );
fclose($f);
$names = array();
...
$f = fopen($resultfolder."namen_3.txt",'w');
fwrite( $f, implode("\n",array_keys($names)) );
fclose($f);
...
$orte = array();
$file = "/Temp/weikinn/101216_Ortsdatei.xls";
...
$f = fopen($resultfolder."alle_namen_unique.txt",'w');
fwrite( $f, $datadump );
fclose($f);
...
$f = fopen($resultfolder."anzahl_texte_jahre.txt",'w');
fwrite( $f, $datadump );
fclose($f);
